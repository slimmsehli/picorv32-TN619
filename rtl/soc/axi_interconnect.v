...
//    0x400F_0000 – 0x400F_00FF   IOMUX  AXI4-Lite
// =============================================================================
`timescale 1ns/1ps
...
module axi_interconnect (
    ...
    // IOMUX AXI4-Lite
    output reg  [31:0] iomux_awaddr,  output reg  iomux_awvalid,  input wire iomux_awready,
    output reg  [31:0] iomux_wdata,   output reg  [ 3:0] iomux_wstrb,
    output reg         iomux_wvalid,  input wire  iomux_wready,
    input  wire [ 1:0] iomux_bresp,   input wire  iomux_bvalid,   output reg  iomux_bready,
    output reg  [31:0] iomux_araddr,  output reg  iomux_arvalid,  input wire  iomux_arready,
    input  wire [31:0] iomux_rdata,   input wire  [ 1:0] iomux_rresp,
    input  wire        iomux_rvalid,  output reg  iomux_rready
);
    ...
    wire sel_iomux = (mem_addr[31:12] == 20'h400F0);  // 0x400F_0000 – 0x400F_0FFF
    ...
    reg [2:0] state;
    reg sel_uart_r;
    reg [1:0] sel_periph_r;  // 0=uart, 1=pwm, 2=iomux
    ...
    always @(posedge clk) begin
        if (!resetn) begin
            ...
            iomux_awvalid<= 0; iomux_wvalid <= 0; iomux_bready <= 0;
            iomux_arvalid<= 0; iomux_rready <= 0;
        end else begin
            mem_ready <= 1'b0;
            ...
            case (state)
                ...
                S_IDLE: begin
                    if (mem_valid && !mem_ready) begin
                        ...
                        else if (sel_uart || sel_pwm || sel_iomux) begin
                            sel_uart_r   <= sel_uart;
                            sel_periph_r <= sel_uart  ? 2'd0 :
                                            sel_pwm   ? 2'd1 : 2'd2;
                            if (is_write) begin
                                if (sel_uart) begin
                                    uart_awaddr  <= mem_addr; uart_awvalid <= 1'b1;
                                    uart_wdata   <= mem_wdata; uart_wstrb  <= mem_wstrb;
                                    uart_wvalid  <= 1'b1;
                                end else if (sel_pwm) begin
                                    pwm_awaddr   <= mem_addr; pwm_awvalid  <= 1'b1;
                                    pwm_wdata    <= mem_wdata; pwm_wstrb   <= mem_wstrb;
                                    pwm_wvalid   <= 1'b1;
                                end else begin
                                    iomux_awaddr <= mem_addr; iomux_awvalid<= 1'b1;
                                    iomux_wdata  <= mem_wdata; iomux_wstrb <= mem_wstrb;
                                    iomux_wvalid <= 1'b1;
                                end
                                state <= S_AXI_ADDR;
                            end else begin
                                if (sel_uart) begin
                                    uart_araddr  <= mem_addr; uart_arvalid <= 1'b1;
                                    uart_rready  <= 1'b1;
                                end else if (sel_pwm) begin
                                    pwm_araddr   <= mem_addr; pwm_arvalid  <= 1'b1;
                                    pwm_rready   <= 1'b1;
                                end else begin
                                    iomux_araddr <= mem_addr; iomux_arvalid<= 1'b1;
                                    iomux_rready <= 1'b1;
                                end
                                state <= S_AXI_READ;
                            end
                ...
                S_AXI_ADDR: begin
                    if (sel_periph_r == 2'd0) begin
                        ...
                        if ((uart_awready || !uart_awvalid) &&
                            ...
                            uart_bready <= 1'b1; state <= S_AXI_RESP;
                        end
                    end else if (sel_periph_r == 2'd1) begin
                        if (pwm_awready)  pwm_awvalid  <= 1'b0;
                        if (pwm_wready)   pwm_wvalid   <= 1'b0;
                        if ((pwm_awready || !pwm_awvalid) &&
                            (pwm_wready  || !pwm_wvalid)) begin
                            pwm_bready <= 1'b1; state <= S_AXI_RESP;
                        end
                    end else begin
                        if (iomux_awready) iomux_awvalid <= 1'b0;
                        if (iomux_wready)  iomux_wvalid  <= 1'b0;
                        if ((iomux_awready || !iomux_awvalid) &&
                            (iomux_wready  || !iomux_wvalid)) begin
                            iomux_bready <= 1'b1; state <= S_AXI_RESP;
                        end
                    end
                end

                S_AXI_RESP: begin
                    if (sel_periph_r == 2'd0 && uart_bvalid) begin
                        uart_bready <= 1'b0; mem_ready <= 1'b1;
                        mem_rdata   <= 32'h0; state    <= S_IDLE;
                    end else if (sel_periph_r == 2'd1 && pwm_bvalid) begin
                        pwm_bready  <= 1'b0; mem_ready <= 1'b1;
                        mem_rdata   <= 32'h0; state    <= S_IDLE;
                    end else if (sel_periph_r == 2'd2 && iomux_bvalid) begin
                        iomux_bready<= 1'b0; mem_ready <= 1'b1;
                        mem_rdata   <= 32'h0; state    <= S_IDLE;
                    end
                end
                ...
                S_AXI_READ: begin
                    if (sel_periph_r == 2'd0) begin
                        ...
                        if (uart_rvalid) begin
                            uart_rready <= 1'b0; mem_ready <= 1'b1;
                            mem_rdata   <= uart_rdata; state <= S_IDLE;
                        end
                    end else if (sel_periph_r == 2'd1) begin
                        if (pwm_arready) pwm_arvalid <= 1'b0;
                        if (pwm_rvalid) begin
                            pwm_rready <= 1'b0; mem_ready <= 1'b1;
                            mem_rdata  <= pwm_rdata; state <= S_IDLE;
                        end
                    end else begin
                        if (iomux_arready) iomux_arvalid <= 1'b0;
                        if (iomux_rvalid) begin
                            iomux_rready <= 1'b0; mem_ready <= 1'b1;
                            mem_rdata    <= iomux_rdata; state <= S_IDLE;
                        end
                    end
                end
